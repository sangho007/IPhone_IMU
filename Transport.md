...
- **전송 프로토콜**: TCP (usbmuxd/iproxy를 통해 `localhost`에서 수신자와 터널링)  
  - UDP 대비 패킷 순서 보장과 신뢰성 확보를 위해 선택
- **호스트 주소**: `127.0.0.1`  
  - 실 기기 ↔ 맥 간 USB 연결 후 `iproxy 4820 4820` 실행을 가정
- **포트**: `4820`  
  - 수신 측에서 동일 포트로 listen 후 protobuf 디코더를 붙인다.

## 페이로드 포맷

- **전송 주기**: 최소 30 Hz  
  - 앱 내부 타이머가 1/30초마다 DTO를 인코딩하며, 센서 이벤트와 실시간성 균형 유지
- **프레이밍**: 길이 + 본문  
  - 스트림 경계를 구분하기 위해 길이 헤더를 선행한다.
...
- **메시지 직렬화**: 커스텀 프로토콜 버퍼 인코더  
  ...
  - SwiftProtobuf 의존성을 제거해 iOS 번들 크기 및 초기화 비용 최소화
...
- iProxy는 USB 케이블 연결이 끊기면 즉시 세션이 종료되므로, 연결 상태 모니터링을 병행 추천
